...
export class StoresListComponent implements OnInit {
  ...
  activeFlag(st_id, isActive) {
    this.storeService.activeFlag({
      isActive: !isActive,
      st_id: st_id
    }).then((response: any) => {
      if (response.status) {
  ...
  removeEzoneStore(index, g_id, e_id, st_id) {
    this.storeService.deleteStore({ g_id: g_id, e_id: e_id, st_id: st_id }, '/v1/store/delete/').then((response: any) => {
      if (response.status) {
        this.getAppStore();
      }
    })
  ...
  assignEngagementZone(index) {
    if (this.store_type == 'APP_STORE') {
      this.storeService.assignEngagementzone({
        st_id: this.stores[index].st_id,
        g_id: this.selected_ezone.g_id,
        e_id: this.selected_ezone.e_id,
      }, "/v1/app-store/assign-ezone/").then((response: any) => {
        if (response.status) {
          this.getAppStore();
        }
      })
      ...
      this.storeService.assignEngagementzone({
        ...
        g_id: this.selected_ezone.g_id,
        e_id: this.selected_ezone.e_id,
      }, "/v1/store/assign-ezone/").then((response: any) => {
        if (response.status) {
          this.getAppStore();
        }
      })
